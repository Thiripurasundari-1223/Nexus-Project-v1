...
namespace PolicyManagement.API.AuthorizationMiddleWare
{
        ...
        public Task Invoke(HttpContext httpContext)
        {
            if (httpContext.Request.Path.Value!.StartsWith("/api/"))
            {
                if (httpContext.User.Claims.Count() > 0)
                {
                    ...
                    using SqlConnection thisConnection = new(this.configuration.GetConnectionString("PMSNexus"));
                    thisConnection.Open();
                    SqlCommand cmd = new("usp_CheckUserAuthorization", thisConnection)
                    {
